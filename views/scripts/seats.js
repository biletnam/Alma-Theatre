...
if(selected_seats === undefined){
	var selected_seats = [];
	$('input[name="seats"]').val(selected_seats.join(','));
}
...
;(function(window) {
	...
	var support = {transitions : Modernizr.csstransitions},
		...
		// main container
		container = document.querySelector('#seats'),
	...
	function selectSeat(planseat) {
		...
		if( classie.has(planseat, 'row__seat--selected') ) {
			...
			for(var i=0; i<selected_seats.length; i++){
				if(selected_seats[i] == seat.dataset.seat){
					selected_seats.splice(i, 1);
					$('input[name="seats"]').val(selected_seats.join(','));
					break;
				}
			}
		...
		// check if exceed maximum ticket number
		if( selected_seats.length >= ticket_amount ) {
			return false;
		}
		...
		// append seat to form
		selected_seats.push(seat.dataset.seat);
		$('input[name="seats"]').val(selected_seats.join(','));
